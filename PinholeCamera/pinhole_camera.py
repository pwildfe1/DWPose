...
class PinholeCamera:
    ...
    def __init__(self, width, height, fx, fy, px = 0, py = 0):
        self.width = width
        self.height = height
        ...
        self.Pinv = np.vstack((self.P, [0, 0, 0, 1]))
        self.Pinv = np.linalg.inv(self.Pinv)
    ...
    def calculate_xyz(self, px, z):

        pixel = np.array([px[0] - self.width/2, px[1] - self.height/2, 1, 1/z])
        position = np.matmul(z*self.Pinv, pixel)
